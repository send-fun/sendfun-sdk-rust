...
use crate::constants::{
	ATA_PROGRAM_ID, DEX_PROGRAM_ID, SYSTEM_PROGRAM_ID, TOKEN_2022_PROGRAM_ID,
};
...
pub fn buy_exact_in(
	...
	let q = amm::buy_exact_in_with_fees(amm::BuyArgs {
		amm: amm::AmmInput {
			quote_reserves: p.quote_reserves,
			base_reserves: p.base_reserves,
...
pub struct TradeArgs {
	/// A [`utils::TradeDirection`](crate::utils::TradeDirection) converts to
	/// this type with `.into()`.
...
pub fn trade_instruction(
	...
	match (direction, mode) {
		(TradeDirection::Buy, TradeMode::ExactIn) => {
			dex_trade_accounts!(super::instructions::BuyExactIn, accounts)
				.instruction(super::instructions::BuyExactInInstructionArgs {
					amount_in: amount,
		...
		(TradeDirection::Buy, TradeMode::ExactOut) => {
			dex_trade_accounts!(super::instructions::BuyExactOut, accounts)
				.instruction(super::instructions::BuyExactOutInstructionArgs {
					amount_out: amount,
					max_amount_in: limit,
				})
		...
		(TradeDirection::Sell, TradeMode::ExactIn) => {
			dex_trade_accounts!(super::instructions::SellExactIn, accounts)
				.instruction(super::instructions::SellExactInInstructionArgs {
					amount_in: amount,
					min_amount_out: limit,
				})
		...
		(TradeDirection::Sell, TradeMode::ExactOut) => {
			dex_trade_accounts!(super::instructions::SellExactOut, accounts)
				.instruction(super::instructions::SellExactOutInstructionArgs {
					amount_out: amount,
					max_amount_in: limit,
				})
...
macro_rules! dex_trade_accounts {
...
fn derive(
	...
	let (pool, _) =
		super::pda::find_pool_pda(state.base_mint, state.quote_mint);
...
mod tests {
	...
	use super::{
		...
		BuildSellExactOutParams, SellExactInParams, TradeArgs,
		...
		sell_exact_in, trade_account_metas, trade_data, trade_instruction,
	};
	use crate::constants::{
		ATA_PROGRAM_ID, DEFAULT_PARTNER, DEX_PROGRAM_ID, SYSTEM_PROGRAM_ID,
		TOKEN_2022_PROGRAM_ID, TOKEN_PROGRAM_ID, WSOL_MINT,
	};
	use crate::dex::events::TradeEvent;
	use crate::dex::types::TradeDirection;
	use crate::utils::{TradeAccounts, TradeMode};
	use solana_instruction::AccountMeta;

	const POOL_INDEX: usize = 2;
	...
	fn assert_derived_market_accounts(
		...
		assert_eq!(ix.accounts[POOL_INDEX].pubkey, pool);
	...
	#[test]
	fn sell_exact_in_builds_from_a_trade_event() {
		let (pool, _) = crate::dex::pda::find_pool_pda(&BASE_MINT, &WSOL_MINT);
		let event = TradeEvent {
			...
			base_reserves: 400_000_000_000,
			quote_reserves: 25_000_000_000,
			timestamp: 0,
		};

		let (ix, quote) = sell_exact_in(&SellExactInParams {
			user: &USER,
			payer: None,
		...
		assert_eq!(quote.base_from_user, 10_000_000_000);
		assert_eq!(quote.fee, 6_097_561);
		assert_eq!(quote.quote_to_user, 603_658_536);

		assert_derived_market_accounts(&ix, &WSOL_MINT, &TOKEN_PROGRAM_ID);
		assert_eq!(ix.accounts[POOL_INDEX].pubkey, event.pool);
		...
		assert_eq!(
			ix.data[..8],
			crate::dex::instructions::SELL_EXACT_IN_DISCRIMINATOR
		);
		let args = <crate::dex::instructions::SellExactInInstructionArgs as borsh::BorshDeserialize>::try_from_slice(&ix.data[8..]).unwrap();
		assert_eq!(args.amount_in, 10_000_000_000);
		// 603_658_536 * 9_950 / 10_000, floored.
		assert_eq!(args.min_amount_out, 600_640_243);
	}
	...
	macro_rules! by_hand {
		($ty:ident) => {
			crate::dex::instructions::$ty {
				user: ACCOUNTS.user,
				payer: ACCOUNTS.payer,
				pool: ACCOUNTS.market,
				...
				associated_token_program: ATA_PROGRAM_ID,
				system_program: SYSTEM_PROGRAM_ID,
				event_authority: crate::dex::pda::EVENT_AUTHORITY_ADDRESS,
				program: DEX_PROGRAM_ID,
			}
		};
	}
